...
import com.fiap.mecanica.adapter.in.web.error.GlobalExceptionHandler;
...
@Import({
        ...
        SecurityEndpointAuthorizationTest.MockConfig.class
})
...
class SecurityEndpointAuthorizationTest {
    ...
    @Autowired
    private AtendimentoUseCase atendimentos;
    @Autowired
    private TemplateUseCase templates;
    @Autowired
    private FuncionarioUseCase funcionarios;

    @BeforeEach
    void setUp() {
        reset(atendimentos, templates, funcionarios);
    }

    @Test
    void atendentePodeIniciarAtendimento() throws Exception {
        when(atendimentos.iniciarAtendimento(eq(1L), eq(2L), eq("Nao liga"), any(), any()))
    ...
    @Test
    void consultaPorIdCombinaPermitAllComRestricaoDeMetodo() throws Exception {
        when(atendimentos.buscarPorId(ORDEM_ID)).thenReturn(ordem(Status.RECEBIDA));
    ...
    @Test
    void funcionarioSemSecuredAceitaQualquerUsuarioAutenticado() throws Exception {
        when(funcionarios.buscarFuncionarioPorId(1L)).thenReturn(funcionario());

        mockMvc.perform(get("/funcionarios/{id}", 1L))
    ...
    void relatorioTempoMedioServicosEhRestritoAoAdmin() throws Exception {
        when(atendimentos.listarTempoMedioExecucaoServicos()).thenReturn(List.of());
    ...
    void mecanicoPodeIniciarDiagnostico() throws Exception {
        when(atendimentos.iniciarDiagnostico(ORDEM_ID)).thenReturn(ordem(Status.EM_DIAGNOSTICO));
    ...
    void mecanicoPodePostarDiagnostico() throws Exception {
        when(atendimentos.realizarDiagnostico(eq(ORDEM_ID), any(), any(), eq("Obs")))
    ...
    void atendentePodeAprovarOrdemServico() throws Exception {
        when(atendimentos.aprovarOrdemServico(ORDEM_ID)).thenReturn(ordem(Status.EM_EXECUCAO));
    ...
    void atendentePodeCancelarOrdemServico() throws Exception {
        when(atendimentos.cancelarOrdemServico(ORDEM_ID)).thenReturn(ordem(Status.CANCELADA));
    ...
    void mecanicoPodeFinalizarOrdemServico() throws Exception {
        when(atendimentos.finalizarOrdemServico(eq(ORDEM_ID), any())).thenReturn(ordem(Status.FINALIZADA));
    ...
    void atendentePodeEntregarOrdemServico() throws Exception {
        when(atendimentos.entregarVeiculo(ORDEM_ID)).thenReturn(ordem(Status.ENTREGUE));
    ...
    void templatesSaoRestritosAoAdmin() throws Exception {
        when(templates.buscarTodos()).thenReturn(List.of());
    ...
    static class MockConfig {
        ...
        FuncionarioUseCase funcionarioUseCase() {
            return Mockito.mock(FuncionarioUseCase.class);
        }
    }
}
