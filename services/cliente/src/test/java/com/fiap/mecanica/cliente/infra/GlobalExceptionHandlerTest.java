...
class GlobalExceptionHandlerTest {
    ...
    void deveHandleEntidadeNaoEncontradaExceptionComStatusNotFound() {
        ClienteNotFound exception = new ClienteNotFound(1L);
    ...
    @Test
    void deveHandleVeiculoNaoEncontradoExceptionComStatusNotFound() {
        com.fiap.mecanica.cliente.exception.VeiculoNaoEncontradoException exception =
                new com.fiap.mecanica.cliente.exception.VeiculoNaoEncontradoException(1L);

        ResponseEntity<RespostaErro> response = handler.handleNotFound(exception);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(response.getBody().erros().getFirst().descricao())
                .isEqualTo("Veículo não encontrado. ID: 1");
    }

    @Test
    void deveHandleConflitoExceptionComStatusConflict() {
        ConflitoException exception = new ClienteExistente("12345678900");
    ...
    void deveHandleAuthenticationExceptionComStatusUnauthorized() {
        org.springframework.security.core.AuthenticationException exception =
                new org.springframework.security.authentication.BadCredentialsException("Erro");
    ...
    void deveHandleAccessDeniedExceptionComStatusForbidden() {
        org.springframework.security.access.AccessDeniedException exception =
                new org.springframework.security.access.AccessDeniedException("Erro");
