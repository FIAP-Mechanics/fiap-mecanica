...
class EstoqueControllerTest {
    ...
    void deveListarInsumosAtivosDoEstoque() {
        when(estoqueUseCase.listarEstoque()).thenReturn(List.of(criarEstoque(true)));
    ...
    void deveExecutarOperacoesDoEstoquePeloIdDoInsumo() {
        ...
        when(estoqueUseCase.buscarPorIdInsumo(1L)).thenReturn(ativo);
        when(estoqueUseCase.cadastrarEstoque(any())).thenReturn(ativo);
        when(estoqueUseCase.atualizarQuantidade(1L, 20L)).thenReturn(ativo);
        when(estoqueUseCase.atualizarInsumo(eq(1L), any())).thenReturn(ativo);
        when(estoqueUseCase.excluirEstoque(1L)).thenReturn(inativo);
        when(estoqueUseCase.reativarEstoque(1L)).thenReturn(ativo);
        ...
        verify(estoqueUseCase).cadastrarEstoque(captor.capture());
        assertThat(captor.getValue().getInsumo().getNome()).isEqualTo("Oleo");
        assertThat(captor.getValue().getQuantidadeInsumo()).isEqualTo(10L);

        ArgumentCaptor<AtualizarInsumoCommand> commandCaptor = ArgumentCaptor.forClass(AtualizarInsumoCommand.class);
        verify(estoqueUseCase).atualizarInsumo(eq(1L), commandCaptor.capture());
        assertThat(commandCaptor.getValue().nome()).isEqualTo("Filtro");
    }

    @Test
    void deveDeduzirEstoqueComSucesso() {
        List<DeduzirEstoqueItemRequest> itens = List.of(new DeduzirEstoqueItemRequest(1L, 5L));

        controller.deduzir(itens);

        verify(estoqueUseCase).deduzirEstoque(any());
    }

    private CadastrarEstoqueRequest criarRequest() {
        return new CadastrarEstoqueRequest(
                CadastrarInsumoRequest.builder().nome("Oleo").precoUnitario(new BigDecimal("45.90")).build(), 10L);
    }
